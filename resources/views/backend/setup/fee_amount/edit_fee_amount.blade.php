		 ...
		 <!-- Basic Forms -->
		  <div class="box">
			...
			<div class="box-body">
			  <div class="row">
				<div class="col">
					<form method="post" action="{{ route('update.fee.amount', $editdata[0]->student_fee_id) }}">
						@csrf
								...
								<div class="form-group">
									...
									<div class="controls">
										<select name="class[]" id="select" required="" class="form-control" aria-invalid="false">
											<option value="{{$edit['student_class']['id']}}">{{$edit['student_class']['name']}}</option> <!-- this method came from fee categoryamount models -->
										</select>
									<div class="help-block"></div></div>
								</div>
					 		</div> <!-- end col md 5 -->
					...
					</div>  <!-- end for class delete_whole_extra_item_Add-->
					 @endforeach
